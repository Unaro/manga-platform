...
export function registerUserEventHandlers(eventBus: EventBus): void {
  ...
  eventBus.subscribe("user.profile.updated", UserEventHandlers.handleUserProfileUpdated);
  eventBus.subscribe("user.deleted", UserEventHandlers.handleUserDeleted);
  eventBus.subscribe("user.email.verified", UserEventHandlers.handleUserEmailVerified);
}