// src/modules/catalog/repositories/supabase/work.repository.ts
...
/**
 * Supabase implementation of Work repository
 */
export class WorkRepositorySupabase implements IWorkRepository {
  constructor(private supabase: SupabaseClient<Database>) {}

  /**
   * Convert Supabase row to domain Work model
   */
  ...
  /**
   * Find work with all relations (authors, genres, tags, sources)
   */
  async findByIdWithRelations(id: string): Promise<WorkWithRelations | null> {
    const { data: workData, error: workError } = await this.supabase
      .from("works")
      .select("*")
      .eq("id", id)
      .single();
    ...
    return {
      ...
      authors: authorsData?.map(a => ({
        id: (a.authors as any).id,
        name: (a.authors as any).name,
        slug: (a.authors as any).slug,
        bio: (a.authors as any).bio,
        createdAt: new Date()
      })) ?? [],
      genres: genresData?.map(g => ({
        id: (g.genres as any).id,
        name: (g.genres as any).name,
        slug: (g.genres as any).slug,
        description: (g.genres as any).description,
        createdAt: new Date()
      })) ?? [],
      tags: tagsData?.map(t => ({
        id: (t.tags as any).id,
        name: (t.tags as any).name,
        slug: (t.tags as any).slug,
        description: (t.tags as any).description,
        category: (t.tags as any).category,
        createdAt: new Date()
      })) ?? [],
      sources: sourcesData?.map(s => ({
        id: s.id,
        sourceName: (s.sources as any).name,
        sourceSlug: (s.sources as any).slug,
        externalId: s.external_id,
        externalUrl: s.external_url,
        externalRating: s.external_rating ?? null,
        externalRatingCount: s.external_rating_count ?? null,
        syncedAt: s.synced_at ? new Date(s.synced_at) : new Date()
      })) ?? [],
      statistics: {
  ...
  /**
   * Find many works with filters and pagination
   */
  async findMany(params: FindManyWorksParams): Promise<{
    works: WorkSummary[];
    total: number;
  }> {
    ...
    const works: WorkSummary[] = (data ?? []).map(row => ({
  ...
  /**
   * Search works by title (full-text search using LIKE)
   */
  ...
  async create(data: CreateWorkData): Promise<Work> {
    const insertData: SupabaseWorkInsert = {
      title: data.title,
      slug: data.slug,
      ...
      alternative_titles: data.alternativeTitles as any,
  ...
  async update(id: string, data: UpdateWorkData): Promise<Work> {
    ...
    if (data.alternativeTitles !== undefined) {
      updateData.alternative_titles = data.alternativeTitles as any;
    }
  ...
  /**
   * Check if work with slug exists
   */
